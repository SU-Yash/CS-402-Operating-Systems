...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
	...
	*result=NULL;
	while(1){
		newpframe=pframe_get_resident(o,pagenum);
		if(newpframe==NULL){
			if(pageoutd_needed()){
				pageoutd_wakeup();
			}
			newpframe=pframe_alloc(o,pagenum);
			if(newpframe==NULL){
				*result=NULL;
				return -1;
			}
			else{
				int res=pframe_fill(newpframe);
				if(res==0){
					*result=newpframe;
				}
				return res;				
			...
			if(!pframe_is_busy(newpframe))
			{
				*result=newpframe;
				return 0;
			}
			while(pframe_is_busy(newpframe)){
				newpframe=pframe_get_resident(o,pagenum);
			}
		}
	}
...
pframe_pin(pframe_t *pf)
{
	...
	int found=0;
	pframe_t* pframe;
	list_iterate_begin(&pinned_list, pframe, pframe_t, pf_link){
        	if((pframe->pf_obj == pf->pf_obj) && (pframe->pf_pagenum == pf->pf_pagenum)){
			found=1;
		}
